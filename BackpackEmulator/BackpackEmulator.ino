...
const int s2_points[] = {988, 1116, 1244, 1372, 1500, 1670, 1841, 2012}; // 8 channels (CH5=1500)
...
String getVtxHtml() {
    String html = "<h3>VTX Table (MHz)</h3><div style='overflow-x:auto;'><table><tr><th>Band</th>";
...
"<style>body{font-family:sans-serif;margin:15px;}input[type=text],input[type=number],select{padding:6px;width:100%;box-sizing:border-box;} .btn{padding:10px 15px;background:#007bff;color:#fff;border:none;border-radius:4px;cursor:pointer;text-decoration:none;display:inline-block;} .btn-sec{background:#6c757d;} table{border-collapse:collapse;width:100%;} td,th{border:1px solid #ccc;padding:4px; text-align:center;}</style></head>"
...
void handleRoot() {
    ...
    html += "<div><label>Binding UID:</label><input type='text' name='uid' value='" + String(uid_buf) + "' pattern='[0-9A-Fa-f]{12}'></div>";
    html += "<div style='display:flex;gap:10px;'><div><label>Video CH (S2):</label><input type='number' name='ch_v' value='" + String(config.ch_v) + "'></div><div><label>Band CH (S3):</label><input type='number' name='ch_b' value='" + String(config.ch_b) + "'></div></div>";
    html += "<div><label>L-Band Mode:</label><select name='l_grid'><option value='0' " + String(config.l_grid==0?"selected":"") + ">Grid 1</option><option value='1' " + String(config.l_grid==1?"selected":"") + ">Grid 2 (Remap to X)</option></select></div>";
...
void handleSave() {
    if (server.hasArg("uid")) {
        ...
        if (s.length() == 12) for(int i=0; i<6; i++) config.uid[i] = strtol(s.substring(i*2, i*2+2).c_str(), NULL, 16);
    }
...
int get_nearest_index(int value, const int points[], int count) {
    ...
    return (min_dist < 60) ? nearest : -1; // 60us deadband
}
...
void setup() {
    ...
    WiFi.softAP("Backpack-Emul"); server.on("/", handleRoot); server.on("/save", HTTP_POST, handleSave); server.on("/bind", handleBind); server.begin();
...
void loop() {
    ...
    if (crsf.updated) {
        ...
        if (v_idx >= 0 && v_idx < 16 && enabled_count > 0) {
            ...
            if (pos != -1) {
                // Map the discrete position to the index of enabled channels
        ...
        int current_band = -1;
        if (b_idx >= 0 && b_idx < 16) {
            int pos = get_nearest_index(crsf.channels[b_idx], s3_points, 6);
            if (pos != -1) current_band = pos;
        }
        ...
        if (current_ch != -1 && current_band != -1) {
            if (current_ch != last_channel || current_band != last_band) {
                ...
                Serial.printf("CRSF S2:%d S3:%d | Switch: B%d C%d (%d MHz)\n",
                              crsf.channels[v_idx], crsf.channels[b_idx], current_band, current_ch+1, config.vtx_table[log_band][current_ch]);
            }
        }
        crsf.updated = false;
    }
