...
static void crsf_task(void *pvParameters) {
    ...
    while (1) {
        ...
        if (len > 0) {
            for (int i = 0; i < len; i++) {
                if (data[i] == 0xC8) { // Sync byte (to master)
                    if (i + 1 < len) {
                        uint8_t frame_len = data[i+1];
                        if (frame_len >= 2 && frame_len <= 62) {
                            if (i + frame_len + 2 <= len) {
                                uint8_t type = data[i+2];
                                uint8_t expected_crc = data[i + frame_len + 1];
                                uint8_t actual_crc = crsf_crc8(&data[i+2], frame_len - 1);
                                if (expected_crc == actual_crc) {
                                    if (type == CRSF_FRAMETYPE_RC_CHANNELS_PACKET) {
                                        crsf_parse_channels(&data[i+3]);
                                    }
                                    i += frame_len + 1;
                                }
                            }
