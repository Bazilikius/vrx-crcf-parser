...
#define WS2812_T0H_NS (250)
#define WS2812_T0L_NS (750)
#define WS2812_T1H_NS (750)
#define WS2812_T1L_NS (250)
...
led_strip_t * led_strip_init(uint8_t channel, uint8_t gpio, uint16_t led_num)
{
    ...
    led_strip_config_t strip_config =  { .max_leds = led_num,.dev = (led_strip_dev_t)config.channel};
...
void ws2812_init(void)
{
    ...
    ESP_ERROR_CHECK(rmt_driver_install(config.channel, 0, 0));
    // install ws2812 driver
    led_strip_config_t strip_config = { .max_leds = STRIP_LED_NUMBER,.dev = (led_strip_dev_t)config.channel};
