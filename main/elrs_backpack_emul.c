#include "elrs_backpack_emul.h"
#include "esp_wifi.h"
#include "esp_now.h"
#include "esp_log.h"
#include <string.h>

static const char *TAG = "BACKPACK_EMUL";
static uint8_t vrx_mac[6] = {0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF};

// MSP v2 Protocol Constants
#define MSP_V2_HEADER_START '$'
#define MSP_V2_HEADER_X     'X'
#define MSP_SET_VTX_CONFIG  0x0059

static uint8_t crc8_dvb_s2(uint8_t crc, uint8_t data) {
    static const uint8_t table[256] = {
        
    };
    return table[crc ^ data];
}

void backpack_emul_init(const uint8_t uid[6]) {
    esp_now_peer_info_t peer = {};
    memcpy(peer.peer_addr, vrx_mac, 6);
    peer.channel = 1;
    peer.ifidx = WIFI_IF_STA;
    esp_now_add_peer(&peer);
}

void backpack_emul_send_vtx_config(uint8_t channel_index) {
    uint8_t payload[4] = {channel_index, 0, 0, 0};
    uint16_t func = MSP_SET_VTX_CONFIG;
    uint16_t size = sizeof(payload);

    uint8_t buf[250];
    buf[0] = '$'; buf[1] = 'X'; buf[2] = '<'; buf[3] = 0;
    buf[4] = func & 0xFF; buf[5] = (func >> 8) & 0xFF;
    buf[6] = size & 0xFF; buf[7] = (size >> 8) & 0xFF;
    memcpy(&buf[8], payload, size);

    uint8_t crc = 0;
    for (int i = 3; i < 8 + size; i++) crc = crc8_dvb_s2(crc, buf[i]);
    buf[8 + size] = crc;

    esp_now_send(vrx_mac, buf, 9 + size);
}
